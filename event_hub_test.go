...
func TestEventHubCloseUnblocksBlockedPublisher(t *testing.T) {
	hub := newEventHub()
	events, _, err := hub.subscribe(SubscriptionPolicy{Buffer: 1, Mode: SubscriptionModeBlock})
	if err != nil {
...
func TestEventHubProcessDiedPublishesAndCloses(t *testing.T) {
	hub := newEventHub()
	events, _, err := hub.subscribe(SubscriptionPolicy{Buffer: 2, Mode: SubscriptionModeRing})
	if err != nil {
		t.Fatalf("subscribe failed: %v", err)
	}
